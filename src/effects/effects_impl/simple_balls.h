...
public:
    ...
    void on_init() {
        ...
        for (uint8_t j = 0; j < balls; j++) {
            ...
            coord[j][1] = LedMatrix.height() / 2 * 10;
    ...
    void on_update() {
        ...
        for (uint8_t j = 0; j < balls; j++) {
            ...
            if (coord[j][0] > (int)(LedMatrix.width() - 1) * 10) {
                coord[j][0] = (int)(LedMatrix.width() - 1) * 10;
                vector[j][0] = -vector[j][0];
            }
            if (coord[j][1] > (int)(LedMatrix.height() - 1) * 10) {
                coord[j][1] = (int)(LedMatrix.height() - 1) * 10;
                vector[j][1] = -vector[j][1];
            }
