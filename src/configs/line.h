...
#define LEDS_WIDTH              (250)                        // ширина матрицы
#define LEDS_HEIGHT             (1)                        // высота матрицы
#define LEDS_MAX_POWER          (3000U)                      // лимит по току в миллиамперах, автоматически управляет яркостью (пожалей свой блок питания!) 0 - выключить лимит
#define LEDS_BRIGHTNRSS         (150)                        // яркость матрицы 0 - 255
#define LEDS_COLOR_ORDER        (RGB)                       // порядок цветов на ленте. Если цвет отображается некорректно - меняйте. Начать можно с RGB

#define LEDS_SYMMETRIC_DEF_STATE false                      // симметричное отображение при старте (если нет сохранённого значения).
                                                            // Логическая матрица вдвое меньше по длинной стороне, нарисованная
                                                            // половина зеркалится на вторую. Переключается из веб-интерфейса.
...
#define BTN_ENABLE              true                        // подключить управление кнопкой (схема: пин — кнопка — GND, внутренняя подтяжка вверх)
...
#define SAVE_SYMMETRIC_FILE         "symmetric.bin"             // симметричное отображение (0/1)
...
#define BTN_PIN                 (12)                        // пин кнопки (второй вывод кнопки на GND)
