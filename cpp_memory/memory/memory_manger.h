...
public:
    ...
    // возвращает область памяти со списком режимов, поэтому
    // нужно обязательно её очистить после использования вызовом delete[]
    uint8_t *get_mod_list() {
        ...
        uint8_t *ids = new uint8_t[mods_amt];
        uint8_t cur_mod = 0;
        while (faddr) {
